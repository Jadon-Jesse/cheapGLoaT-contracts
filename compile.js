...
function createConfiguration() {
    return {
        ...
        sources: {
            '69_CheapGloat.sol': {
                content: fs.readFileSync(path.resolve(__dirname, 'contracts', '69_CheapGloat.sol'), 'utf8')
            },
        },
        settings: {
...
function writeOutput(compiled, buildPath) {
    ...
    for (let contractFileName in compiled.contracts["69_CheapGloat.sol"]) {
        const contractName = contractFileName.replace('.sol', '');
        console.log('Writing: ', contractName + '.json');
        fs.outputJsonSync(
            path.resolve(buildPath, contractName + '.json'),
            compiled.contracts["69_CheapGloat.sol"]
        );
    }
}
